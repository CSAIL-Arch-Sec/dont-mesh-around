...
int main(int argc, char **argv)
{
	...
	// Check arguments
	if (argc != 6) {
		fprintf(stderr, "Wrong Input! Enter desired core ID, slice ID, repetitions-train, repetitions-test, and last iteration of interest!\n");
		fprintf(stderr, "Enter: %s <core_ID> <slice_ID> <repetitions_train> <repetitions_test> <victim_iteration_no_last>\n", argv[0]);
		exit(1);
	}
	...
	// Parse repetitions
	int repetitions_train;
	sscanf(argv[3], "%d", &repetitions_train);
	if (repetitions_train < 0) {
		fprintf(stderr, "Wrong repetitions_train! repetitions_train should be greater than 0!\n");
		exit(1);
	}

	// Parse repetitions
	int repetitions_test;
	sscanf(argv[4], "%d", &repetitions_test);
	if (repetitions_test < 0) {
		fprintf(stderr, "Wrong repetitions_test! repetitions_test should be greater than 0!\n");
		exit(1);
	}

	// Parse last victim iteration to attack
	int victim_iteration_no_last;
	sscanf(argv[5], "%d", &victim_iteration_no_last);
	if (victim_iteration_no_last <= 0) {
		printf("Wrong victim_iteration_no! victim_iteration_no_last should be greater than 0!\n");
		exit(1);
	}
	...
	printf("Now collecting train data\n");

	int rept_index;
	for (rept_index = 0; rept_index < repetitions_train; rept_index++) {

		printf("\r%4d", rept_index);
		fflush(stdout);
		...
		// Collect one trace for each iteration with this key
		int victim_iteration_no;
		for (victim_iteration_no = 1; victim_iteration_no < victim_iteration_no_last + 1; victim_iteration_no++) {
			// Prepare
			uint8_t active = 0;
			...
			uint8_t actual_bit = sharestruct->bit_of_the_iteration_of_interest;

			// Prepare data output file
			char output_data_fn[64];
			sprintf(output_data_fn, "./out-train/%04d_data_%04d_%" PRIu8 ".out", rept_index, victim_iteration_no, actual_bit);
			FILE *output_data;
	...
	printf("Now collecting test data\n");

	// Make it so that the victim switches to the test key
	sharestruct->use_randomized_key = 1;

	for (rept_index = 0; rept_index < repetitions_test; rept_index++) {
		...
		for (victim_iteration_no = 1; victim_iteration_no < victim_iteration_no_last + 1; victim_iteration_no++) {
			...
			// Prepare data output file
			char output_data_fn[64];
			sprintf(output_data_fn, "./out-test/%04d_data_%04d_%" PRIu8 ".out", rept_index, victim_iteration_no, actual_bit);
			FILE *output_data;
			if (!(output_data = fopen(output_data_fn, "w"))) {
				perror("fopen");
				exit(1);
